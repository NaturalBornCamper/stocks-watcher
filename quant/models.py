...
class CompiledScoreBase(models.Model):
    sa_stock = models.ForeignKey(SAStock, on_delete=models.CASCADE, db_index=True)
...
class CompiledScore(CompiledScoreBase):
    class Meta:
        db_table = f"{CompiledScoreBase._meta.model_name}_regular"
        constraints = [
            UniqueConstraint(name="quant__compiled_score__unique__sa_stock__type", fields=["sa_stock", "type"])
        ]


class CompiledScoreDecayed(CompiledScoreBase):
    DECAY_MONTHS = 3

    class Meta:
        db_table = f"{CompiledScoreBase._meta.model_name}_decay"
        constraints = [
